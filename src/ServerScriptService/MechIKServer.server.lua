...
local horizAngle = math.acos( front:Dot(vecOneHoriz)/(front.Magnitude * vecOneHoriz.Magnitude) )
local vertAngle = math.acos( front:Dot(vecOneVert)/(front.Magnitude * vecOneVert.Magnitude) )
...
RunService.Heartbeat:Connect(function()
	...
	limbVectorTable = FabrikAlgo(0.1,hipJointCFrame, goalPosition, limbVectorTable,limbLengthTable)
	...
	local rotateUpperLegAngle = math.acos( vecOneRel:Dot(v1New)/(vecOneRel.Magnitude * v1New.Magnitude) )
	...
	local newUpLegCF = hipCFrame:Inverse()*CFrame.fromMatrix(upperLegPos,upperLegRightVector,-v1New)
	...
	local newKneeCF = upLegCFrame:Inverse()*CFrame.new(kneePos,kneePos-v2New)
	...
	local newLowLegCF = kneeCFrame:Inverse()*CFrame.fromMatrix(lowerLegPos,lowerLegRightVector,-v3New)
